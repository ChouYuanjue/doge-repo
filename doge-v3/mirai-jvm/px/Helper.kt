        ...
        nodes.add(
            ForwardMessage.Node(
                ...
                message = At(event.sender).plus("\n")
                    .plus(" /px rank ${Command.showRank}daily|weekly|monthly|rookie|original|male|female|daily_r18|weekly_r18|male_r18|female_r18|r18g-页码").plus("\n")
                    .plus(" /px id ${Command.getDetailOfId}pixivId").plus("\n")
                    .plus(" /px tst ${Command.picToSearch}你要找的图片").plus("\n")
                    .plus(" /px user ${Command.findUserWorksById}作者id").plus("\n")
                    .plus(" /px tsf ${Command.searchInfoByPic}番剧截图").plus("\n")
                    .plus(" /px setu ${Command.setu}").plus("\n")
                    .plus(" /px setu +tag ${Command.setu} loli").plus("\n")
                    .plus(" /px loli ${Command.lolicon} 查询条件 r18,查询条件最多三个用&分割，|为或者,例：少女|姐姐&黑丝|白丝&猫耳|狗耳;如果有r18则必须放在最后，开启18禁模式").plus("\n")
                    .plus(" /px tag ${Command.tag}关键词-页码").plus("\n")
                    .plus(" /px detect ${Command.detect}[图片]")
            )
        )

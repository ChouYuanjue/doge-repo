...
@register("doge_soup", "runnel", "AI海龟汤推理游戏插件", "1.0.0")
# 线程安全的题库管理基类
...
@register(
    "astrbot_plugin_soup",
    "runnel",
    "AI海龟汤推理游戏插件，支持自动生成谜题、智能判断、验证系统、智能提示、存储库管理等功能。",
    "1.0.0"
)
...
请判断其等级和简评。"""
    ...
    @filter.command("soup level",alias={"汤难度"})
    ...
    # 🎮 开始游戏指令
    @filter.command("soup start",alias={"汤"})
    async def start_soupai_game(self, event: AstrMessageEvent):
        ...
        if self.game_state.is_game_active(group_id):
            ...
            yield event.plain_result(
                "当前群聊已有活跃的海龟汤游戏，请等待游戏结束或使用 /soup reaveal 或 /揭晓 结束当前游戏。"
            )
            return
        ...
        try:
            ...
            if self.game_state.start_game(
                ...
                yield event.plain_result(
                    f"🎮 海龟汤游戏开始！{extra}\n\n📖 题面：{puzzle}\n\n💡 请直接提问或陈述，我会回答：是、否、是也不是\n💡 输入 /soup reveal 或 /揭晓 可以查看完整故事\n💡 输入 /soup hint 或 /提示 可以获取方向性提示"
                )
    ...
    # 🔍 揭晓指令
    @filter.command("soup reveal",alias={"揭晓"})
    async def reveal_answer(self, event: AstrMessageEvent):
        ...
        if not game:
            yield event.plain_result(
                "当前没有活跃的海龟汤游戏，请使用 /soup start 或 /汤 开始新游戏。"
            )
            return
        ...
        try:
            ...
            async def game_session_waiter(
                ...
                try:
                    ...
                    if user_input in ("/汤状态", "汤状态","/soup status"):
                        await self._handle_game_status_in_session(event, group_id)
                        return

                    if user_input in ("/强制结束", "强制结束","/soup end"):
                    ...
                    if user_input.startswith("/验证"):
                        ...
                        return
                    elif user_input.startswith("/soup vf"):
                        import re

                        match = re.match(r"^/soup vf\s*(.+)$", user_input)
                        if match:
                            user_guess = match.group(1).strip()
                            # 手动调用验证函数
                            await self._handle_verification_in_session(
                            ...
                            await event.send(
                                event.plain_result(
                                    "请输入要验证的内容，例如：/soup vf 他是她的父亲"
                                )
                            )
                        return
                    # 特殊处理 /揭晓 指令
                    if user_input in ("揭晓","soup reveal","/soup reveal"):
                        # 获取游戏信息并发送答案
                        game = self.game_state.get_game(group_id)
                        if game:
                            answer = game["answer"]
                    ...
                    if question_limit is not None and question_count >= question_limit:
                        ...
                        await event.send(
                            event.plain_result(
                                f"❗️提问次数已用完，请使用 /验证 或 /soup vf 进行猜测（剩余{remaining}次验证机会）"
                            )
                        )
                        return
    ...
    # 📊 游戏状态查询
    @filter.command("soup status","汤状态")
    ...
    async def global_command_interceptor(self, event: AstrMessageEvent):
        ...
        if self.auto_generating:
            ...
            if (
                    ...
                    and not user_input.startswith("/soup")
            ):
                yield event.plain_result(
    ...
    # ⚙️ 查看当前配置
    @filter.command("soup config","汤配置")

...
class AutoEmojiFlowTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_search_tool_bypasses_probability_gate(self):
        ...
        result_text = "\n".join(results)
        self.assertIn("send_emoji(emoji_id=", result_text)
    ...
    def test_tool_documentation_uses_exposed_names(self):
        search_doc = Main.search_emoji.__doc__ or ""
        send_doc = Main.send_emoji.__doc__ or ""

        self.assertIn("send_emoji", search_doc)
        self.assertIn("search_emoji", send_doc)
        self.assertNotIn("send_meme", search_doc)
        self.assertNotIn("search_meme", send_doc)
    ...
    async def test_steal_tool_passes_origin_metadata_to_processor(self):
        ...
        try:
            results = []
            async for chunk in main.steal_emoji(DummyEvent(), "https://example.test/a.png"):
                results.append(chunk)
        finally:

...
SUBCOMMANDS
  ...
  daily             3 功能  超理日报查看与订阅管理：日报是独立于日常人格/群聊上下文的正式 TeX/PDF 技术简报，主题全集来自官方 #今日活跃，逐主题补首楼与最近24小时楼层证据并生成证据约束的编辑稿；通过一个合并转发发送逐页预览和 PDF 原件。空参数/now 立即发送当前报告；定时日报与实时 push 独立。
  ...
  status           检查 Chaoli 专用选择性代理与论坛首页是否可达。
    /chaoli status
  push             本群订阅超理新动态；新帖与旧帖新回复分别识别。首次开启只建立当前水位，不补发历史；修改订阅仅限本群群主/管理员。
    /chaoli push on [板块] | off [板块] | status | test [板块]
...
DIRECT
  /chaoli daily [now|push|on [HH:MM]|off|status|test]
    超理日报查看与订阅管理：日报是独立于日常人格/群聊上下文的正式 TeX/PDF 技术简报，主题全集来自官方 #今日活跃，逐主题补首楼与最近24小时楼层证据并生成证据约束的编辑稿；通过一个合并转发发送逐页预览和 PDF 原件。空参数/now 立即发送当前报告；定时日报与实时 push 独立。

SUBCOMMANDS
  now              查看超理日报：立即发送当前正式 TeX/PDF 日报到当前群；一个合并转发内包含逐页图片预览和 PDF 原件。报告不经过日常人格或群聊上下文，使用官方 #今日活跃主题全集和逐帖公开证据，不修改定时 last_sent。
    /chaoli daily now
  push             主动触发超理日报推送：立即发送当前正式 TeX/PDF 日报到当前群，一个合并转发内包含逐页预览和 PDF 原件；报告独立于日常人格/群聊上下文；成功后若已启用定时日报则推进当天 last_sent，避免 23:45 重复发送。
    /chaoli daily push
...
```text
COMMAND  /pixiv
Pixiv 插画搜索：标签搜索优先使用 Pixiv 匿名长期热门种子，再由相关推荐扩展高质量候选，近期热门与最新流依次回退；原图 CDN 下载，Lolicon 负责随机/画师与故障回退；固定关闭 R18、过滤 AI 生成作品。

DIRECT
  /pixiv <标签> [数量]
    按标签/关键词搜索 Pixiv 插画；长期热门优先，相关推荐严格保留原标签，近期热门与最新流依次回退，并按会话去重；原图优先；群聊最多 3 张，私聊最多 5 张；固定 R18=off、AI filtered。

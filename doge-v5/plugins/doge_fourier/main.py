...
@register('doge_fourier','runnel','SVG、文本与图片轮廓的傅里叶旋转向量动画','5.1.0')
class DogeFourier(Star):
    def __init__(self, context: Context):
        super().__init__(context)
        self.data_dir=Path(StarTools.get_data_dir('doge_fourier'))
        self.mode_path=self.data_dir/'modes.json'; self.modes=self._load_modes()

    def _load_modes(self):
        try: return json.loads(self.mode_path.read_text()) if self.mode_path.exists() else {}
        except Exception: return {}

    def _save_modes(self):
        self.mode_path.parent.mkdir(parents=True,exist_ok=True)
        tmp=self.mode_path.with_suffix('.tmp'); tmp.write_text(json.dumps(self.modes,ensure_ascii=False,indent=2)); tmp.replace(self.mode_path)

    @staticmethod
    def _uid(event):
        try: return str(event.get_sender_id() or 'default')
        except Exception: return 'default'

    def _mode(self,event): return self.modes.get(self._uid(event),'merge')

    @staticmethod
    def _vf(rest: str) -> tuple[int,int]:
        ...
        if len(vals)>2: raise FourierError('image 最多接受 vectors frames 两个参数')
        ...
        try:
            ...
            if not parts or parts[0].lower() in {'help','?'}:
                yield text_result(event,
                    ...
                    '输出是真正的轮廓 DFT 旋转圆/向量 GIF，不是普通 FFT 频谱图。',markdown=False); return
            action=parts[0].lower(); rest=parts[1] if len(parts)>1 else ''
            if action=='mode':
                value=rest.strip().lower()
                if not value:
                    yield text_result(event,f'当前模式：{self._mode(event)}（merge / separate）',markdown=False); return
                if value not in {'merge','separate'}: raise FourierError('mode 只支持 merge / separate')
                self.modes[self._uid(event)]=value; self._save_modes()
                yield text_result(event,f'已设置 Fourier 模式：{value}',markdown=False); return
            mode=self._mode(event)
            if action in {'image','img','draw'}:
                vectors,frames=self._vf(rest)
                mats=await MATERIALS.resolve(event,'image',needed=1)
                mats=await wait_for_materials(event,'image',1,mats)
                path,stats=await asyncio.to_thread(FourierRenderer.from_image_path,mats[0].path,mode=mode,vectors=vectors,frames=frames)
            ...
            caption=(f'Fourier epicycles · {source} · {stats.mode} · contours={stats.contours} · '
